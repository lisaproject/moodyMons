package com.example.moodymons.feature;
...
public class MFCC {
    ...
    private final static int       hop_length           = 128; // 512//4
    ...
    public float[][]getMelSpec(double[] doubleInputBuffer){
        final double[][] specTroGram = powerToDb(melSpectrogram(doubleInputBuffer));
        float[][]finalMelSpec = new float[specTroGram.length][specTroGram[0].length];
        for (int i = 0; i < specTroGram.length; i++){
            for (int j = 0; j < specTroGram[0].length; j++){
                finalMelSpec[i][j] = (float) specTroGram[i][j];
            }
        }
        return  finalMelSpec;
    }

    public float[] process1d(double[] doubleInputBuffer) {
